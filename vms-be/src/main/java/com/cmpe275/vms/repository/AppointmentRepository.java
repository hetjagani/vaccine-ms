...
public interface AppointmentRepository extends JpaRepository<Appointment, Integer> {
	...
	@Query(value = "SELECT id, status from appointment WHERE user_id=?3 AND date>?1 AND date<?2 ORDER BY date ASC, time ASC;", nativeQuery = true)
	List<Object[]> findAppointmentsBetweenDates(String date1, String date2, String userId);

	@Query(value = "SELECT id, status from appointment WHERE date>?1 AND date<?2 AND clinic_id=?3 ORDER BY date ASC, time ASC;", nativeQuery = true)
